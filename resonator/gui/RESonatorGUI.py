...
import resonator.data.DataPrep
import resonator.data.XMLGenerator
...
logging.basicConfig(filename=os.path.join(home, logging_output), level=logging.DEBUG)
...
class RESonatorGUI:
    ...
    def __init__(self) -> None:
        ...
        self.layout_selections.addWidget(QLabel("Select input files to process:"))

        # Add widgets to the GUI
        self.layout_selections.addWidget(self.make_filepicker_section(data_name="lms"))

        self.lessons = QComboBox()
        self.lessons.currentIndexChanged.connect(lambda: self.change_lesson())
        ...
        self.layout_selections.addWidget(self.make_section_process())
    ...
    def make_filepicker_section(self, data_name):
        ...
        :return:
        """
        self.horizontalGroupBox = QGroupBox("Select input file for " + str(data_name))
        ...
        choose_1.clicked.connect(
            lambda: self.open_file(file_name=data_name, label_to_modify=label_pathname)
        )
    ...
    def open_file(self, file_name, label_to_modify):
        try:
            name = QFileDialog.getOpenFileName(
                caption="Select the .CSV file for" + file_name, filter="csv(*.csv)"
            )
        except:
        ...
        if file_name == "lms":
            try:
                ...
                Exception(
                    "ERROR: Error occurred while reading list of " "lessons: " + str(e)
                )
    ...
    def orchestrate_xml(self):
        ...
        try:
            ...
            prep = DataPrep.DataPrep(
                pd.read_csv(self.paths.get("lms"), encoding="latin1"),
                pd.read_csv(self.paths.get("evaluation"), encoding="latin1"),
            ...
            logging.info("TEST: List of Lessons:" + str(prep.get_lessons_list()))
            ...
            try:  # Add try/except every time a major thing can error
                ...
                QMessageBox.about(
                    self.window, "Error", "Couldn't generate XML. " + str(e)
                )
            # Successfully compiled XML
            QMessageBox.about(
                self.window,
                "Success",
                "Compiled XML as: " + str(generator.string_output_file_path),
            )
        except Exception as e:
            ...
            QMessageBox.about(
                self.window,
                "Error",
                "Error generating output XML. " "Exception: " + str(e),
            )
